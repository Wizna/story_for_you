...
本计划最初以《边城》暴露的问题为例，但实现约束不能写入某一本书。当前续写链路已改为：

- `continue` 先生成结构化 `ContinuationPlan`，由模型根据当前状态、原文证据和读者要求判断承接/展开/转折/高潮/余波/终止阶段，再选择 1 到 `max_chapters` 个叙事单元；最后一个单元只有在证据支持时才承担主要收束。
- 每个计划章节分别生成和润色，全部章节合并后再做跨章伏笔和一致性审查。
- 所有续写阶段共享原文尾部、索引场景尾部、不可逆事实和人物状态；不再只依赖 280 字片段。
- 分析器接收切分器提供的源章节标签；长章拆成多个物理单元时，事件和摘要继续使用源章节号，不再把物理片段数误报成章节号。
- 通用称谓（如“师父”“长老”）发生歧义时不再自动合并人物；明确的新阵营观察会更新人物当前阵营。
- 后处理只删除整段重复文本，保留有相同开头但包含新行动或结果的段落。
- 最终修复根据用户的开放/闭合要求生成，不再包含特定小说人物名。

《英雄志》只作为长篇回归样例：约 350 万字、215 个源章节、613 个分析单元。它的末尾场景包含多视角信息差和未揭示身份，验收时必须检查场景承接与知情范围，不能只检查是否生成了一个“完整结局”。
...
历史问题：早期 `HintInterpreter` 用关键词把“开放”映射为 `OE`，会误读“非开放式结局”。当前实现由模型解析自然语言，并以 `closure`、`continuation_intent` 和 `requested_scope` 表达要求；`ending_direction` 仅作为旧缓存兼容字段。
